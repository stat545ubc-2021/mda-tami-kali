...
```{r}
#reorder the previously low and average values, will create this from the original data set 
...
area_symmetry_plot <- ggplot(symmetry_cancer_cat_fct, aes(area_mean, radius_mean))+ 
  ...
  ggtitle("High Symmetry of Area vs Radius of Maligant Tumors") +
  scale_x_continuous(
    name = "Mean Tumor Area"
  )+
  scale_y_continuous(
...
```
Why you chose this grouping: This was the only categorical value plot that I had, so I addressed this set of data, looking at symmetry. 

justification for the grouping, which could be subtle or speculative: Looking at the previous graph, differentiating between the average and low values did not provide any additional information, so i wanted to 
...
```{r}
#I need to recreate another set of categorical values, because currently I only have either 2 or 3 categorical values, where it will not make sense to reorder any factors. This will be the set up stage for this step 

```

why you chose this ordering:
